document.addEventListener('DOMContentLoaded', () => {
    ...
    let sfxEnabled = true;
    // MODIFICATION: Ajout d'une variable pour garder le contexte de navigation
    let currentNavigationContext = { playlist: [], index: -1 };
    
    // NOUVELLES VARIABLES POUR LA LOGIQUE DE PIÈCES/SUCCÈS
    ...
    function onPlayerStateChange(event) {
        ...
        if (event.data === YT.PlayerState.ENDED) {
            ...
            // CORRECTION: Logique de gain de pièce/succès revue
    ...
    function playVideoWhenReady(item, playlistIds = [], index = -1, fromPlaylistOverlay = false, forceTempPlay = false) {
        ...
        // MODIFICATION: Logique de file d'attente revue pour le bouton "lancer la chanson" et la navigation
        if (forceTempPlay) {
            if (currentNavigationContext && currentNavigationContext.playlist.length > 0) {
                ...
                if (currentQueueIndex === -1) { // Fallback si l'item n'est pas dans le contexte
                    activePlaybackQueue = [item.id];
                    currentQueueIndex = 0;
                }
            } else {
                // Fallback si aucun contexte de navigation n'est disponible
                activePlaybackQueue = [item.id];
                currentQueueIndex = 0;
            }
    ...
    function renderCards(containerId, cardsData, cardType = 'generic') {
        ...
        Object.entries(cardsData).forEach(([key, item]) => {
                  ...
                  <a href="#" class="card-link-wrapper" data-unlock-album="dream-season">
                    <div class="card-inner">
                      <div class="lock-overlay">
                        ...
                        <span class="unlock-cost">${COIN_COST_UNLOCK} <i class="fa-solid fa-circle-dollar"></i></span>
                      </div>
            ...
            if (cardType === 'menu' && activeProfile === 'mmg-music') {
                ...
                let description = '';
                // MODIFICATION : Logique pour ne pas afficher "voir le contenu" sur les cartes menu de mmg-beats
    ...
    function updateProgressBar() {
        ...
        if (duration > 0) {
            ...
            // CORRECTION: Nouvelle logique de détection de seek
            if (previousListenProgress > 0) {
                const progressDifference = listenProgress - previousListenProgress;
                // Un saut de plus de 2 secondes est considéré comme un seek
    ...
    // =========================================================
    // MODIFICATION: Logique de lecture/navigation revue
    const playNextTrack = (direction = 1, forcePlay = false) => {
        ...
        // MODIFICATION: Toujours se baser sur la page affichée pour la navigation
        ...
        // Déterminer la file d'attente à utiliser: celle en cours de lecture, ou le contexte de navigation
        if (activePlaybackQueue.length > 0) {
            currentQueue = activePlaybackQueue;
            // L'index de départ pour la navigation est TOUJOURS celui de la page affichée
            currentIndex = currentQueue.findIndex(id => id === currentItemIdOnPage);
            // Fallback si l'élément affiché n'est pas dans la file active (ne devrait pas arriver)
        ...
        if (currentIndex === -1 || currentQueue.length === 0) return; // Navigation impossible
    
        let nextIndex;
        if (isShuffleMode && (forcePlay || isAutoplayActive)) { // Le shuffle n'a de sens que si on change de piste
            let randomIndex;
            do {
    ...
    function setupEventListeners() {
        ...
        contentBoard.addEventListener('click', (e) => {
            ...
            if (youtubeId) {
                ...
                // MODIFICATION: Sauvegarder le contexte de navigation actuel
        ...
        // MODIFICATION: Les boutons précédent/suivant respectent l'état de l'autoplay
        ...
        const toggleAutoplay = (e) => {
            ...
            // CORRECTION: Si on active l'autoplay et qu'un titre vient de se terminer, on lance le suivant
        ...
        document.getElementById('progress-bar').addEventListener('click', (e) => {
            if (activePlayer && typeof activePlayer.getDuration === 'function') {
                ...
                seekDetectedInCurrentPlay = true; // Un clic sur la barre de progression est un seek
            }
        });
        ...
        function closeOverlay(sound) {
            ...
            // MODIFICATION: Correction de la logique pour ne pas reprendre la musique si on ferme l'overlay du guide
            const keepMusicPlaying = activeOverlay.id === 'tags-filter-overlay' || activeOverlay.id === 'tutorial-overlay';
            ...
            overlay.addEventListener('click', (e) => {
                if (e.target === overlay) {
                    if (overlay.id === 'tutorial-overlay' && isTutorialActive) {
                        return; // Ne pas fermer si le guide est en cours
                    } else {
                       closeOverlay(sounds.back);
                    }
                }
        ...
        let originalAutoplayState = isAutoplayActive;
        ...
        const mainMenuTutorialSteps = [
            ...
            { step: 3, selector: '.search-bar', text: 'Recherchez un titre, un artiste ou un album ici.' },
            { step: 4, selector: '#tags-filter-btn', text: 'Appuyez ici pour filtrer le contenu par tags (ex: retro, pop...).' },
            { step: 5, selector: '#shop-btn', text: 'Visitez la boutique pour débloquer des thèmes visuels.' },
            { step: 6, selector: '#achievements-btn', text: 'Consultez vos succès. Gagnez des pièces en écoutant des morceaux pour débloquer du contenu !' },
            ...
            { step: 12, selector: '#mp3-player-container', text: 'Voici le mini-lecteur. Il apparaît dès qu\'un son est lancé.' },
            { step: 13, selector: '#player-album-cover', text: 'La pochette est cliquable et permet de revenir instantanément au contenu qui joue (musique ou vidéo) !' },
            { step: 14, selector: '#autoplay-toggle', text: 'Activez l\'autoplay pour enchaîner les médias. Désactivez-le pour explorer le site sans interrompre la lecture en cours. (sur mobile : voir les trois points verticaux).' },
            ...
            { step: 17, selector: '.volume-control-new', text: 'Ajustez le volume ici (sur mobile : voir les trois points verticaux).' },
            { step: 18, selector: '#player-like-btn', text: 'Aimez un titre pour le retrouver facilement plus tard.' },
            { step: 19, selector: '#playlist-btn', text: 'Gérez votre playlist actuelle.' },
            { step: 20, selector: '#share-btn', text: 'Partagez le lien de la musique en cours (sur mobile : voir les trois points verticaux).' },
        ...
        const musicTitlesTutorialSteps = [
            {
                step: 1,
                text: "Sur ordinateur, le défilement du titre s'arrête lorsque vous survolez une carte. Sur mobile, il s'arrête lorsque vous maintenez votre doigt appuyé.",
            ...
            },
            {
                ...
                text: "La pochette est cliquable. Cliquez dessus pour l'afficher en grand format.",
                position: 'pos-top',
                reverseAction: () => { 
                    showSection('titles-section');
                }
            ...
            },
            {
                ...
                text: "Sur la droite, vous trouverez les contenus visuels associés, comme le clip ou le making-of.",
                position: 'pos-top',
                action: () => { // Action to prepare for step 8
            ...
            },
            {
                ...
                text: "Ce bouton apparaît lorsque l'Autoplay est désactivé. Il permet de lancer la lecture de ce titre précis, une exception pratique pour ne pas avoir à retourner sur la liste des titres.",
                position: 'pos-top',
                action: () => { // Cleanup for step 9
                    updateTempPlayButtonVisibility();
                },
                reverseAction: () => { // Cleanup for step 7
                    updateTempPlayButtonVisibility();
                }
            },
            {
                ...
                text: "lorsque le bouton Autoplay est désactivé (cf. guide 1), vous pouvez naviguer entre les pages des titres sans que la musique en cours ne change, à l'aide des boutons 'précédent' et 'suivant' du mini-player",
                position: 'pos-top'
            },
            {
                step: 10,
        ...
        function showTutorialStep(stepIndex) {
            if (highlightedElements.length > 0) {
                highlightedElements.forEach(el => {
                    ...
                    if (el.classList.contains('card')) {
                        // MODIFICATION: This line is commented out to prevent the card from flickering/disappearing
                        // when navigating between a card and its child element in the tutorial.
                        // The opacity will be reset globally by the endTutorial() function.
                        // el.style.opacity = '';
                    }
                });
                highlightedElements = [];
            ...
            if (targetElements.length === 0) {
                ...
                // MODIFICATION: Scroll into view smoothly.
            ...
            tutorialBox.className = 'tutorial-box-style'; // generic class to remove pos- classes
        ...
        function startTutorial(guideKey) {
            ...
            // CORRECTION: Logique pour éviter le flash de la fenêtre du guide
            if (guideKey === 'music') {
                // Pour le guide musical, on attend la fin de l'animation de la section
                setTimeout(showFirstStep, 600);
            } else {
                // Pour le guide principal, on l'affiche directement
                showFirstStep();
            }
        }
        ...
        function endTutorial() {
            ...
            if (isAutoplayActive !== originalAutoplayState) {
                toggleAutoplay();
            }

            // MODIFICATION: Reset button visibility on tutorial end
            updateTempPlayButtonVisibility();
