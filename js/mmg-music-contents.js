    ...
    function renderShopItems() {
        ...
        themesContainer.innerHTML = themes.map(item => {
            ...
            const descriptionText = isUnlocked 
                ...
                // CORRECTION: La description est maintenant gérée par la clé de traduction du thème
                : `${getTranslation('completeMission')} "${getTranslation(`achievement_${item.missionId}_title`)}"`;
    ...
    function getCorrectImagePath(item, size = 'full') {
        ...
        // CORRECTION: La logique est ajustée pour utiliser le sous-dossier /thumb/ pour les vignettes.
        if (size === 'thumb') {
            // "assets/pochettes/Pochette-XYZ.webp" devient "assets/pochettes/thumb/Pochette-XYZ.webp"
            const pathParts = item.image.split('/');
            if (pathParts.length > 1) {
                const filename = pathParts.pop();
                const basePath = pathParts.join('/');
                return `${basePath}/thumb/${filename}`;
            }
            // En cas d'échec (si le chemin n'est pas standard), on retourne l'image de base.
            return item.image;
        }
    ...
    function setupEventListeners() {
        ...
        if (shopTabsContainer) {
            shopTabsContainer.addEventListener('click', (e) => {
                ...
                shopTabsContainer.querySelectorAll('.playlist-tab-btn').forEach(btn => btn.classList.remove('active'));
                button.classList.add('active');

                // Afficher le contenu correspondant
                const tabId = button.dataset.tab;
                document.getElementById('shop-themes-content').classList.toggle('hidden', tabId !== 'themes');
                document.getElementById('shop-missions-content').classList.toggle('hidden', tabId !== 'missions');
            });
        }
