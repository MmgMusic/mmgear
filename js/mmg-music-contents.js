    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (selectBtn && selectBtn.classList.contains('locked')) {
            ...
            playAudio(sounds.blocked); // Le son est déjà joué par l'autre listener, mais on le garde en sécurité.
            return;
        }
        if (buyBtn) {
    ...
    function renderDashboardCarousel() {
        ...
        container.innerHTML = latestItems.map(item => {
            ...
            const translatedDesc = getTranslation(`desc_${item.id}`);
            let description = (translatedDesc.startsWith('[') && item.description) ? item.description : translatedDesc;
