    ...
    function renderShopItems() {
        ...
        const shopItems = {
            themes: [
                ...
                { id: 'theme-android', nameKey: 'theme_android', descKey: 'theme_android_desc', icon: 'smartphone', color: 'green', cost: 3, achievement: 'patienceIsKey' },
            ],
            backgrounds: [
                { id: 'default-bg', nameKey: 'bg_bubbles', icon: 'sparks', color: 'blue', cost: 0 },
                { id: 'theme-bg-waves', nameKey: 'bg_waves', icon: 'waves', color: 'yellow', cost: 3 },
                { id: 'theme-bg-gray', nameKey: 'bg_gray', icon: 'square', color: 'gray', cost: 3 },
            ]
        };

        const renderBackgroundItem = (item) => {
            // CORRECTION: On ne vérifie plus si l'item est sélectionné ici. On affiche un bouton d'achat ou un bouton "select" de base.
            const isAcquired = (localStorage.getItem('bg-theme') || 'default-bg') === item.id;
            const colors = previewColors[item.color] || previewColors.gray;
            const buttonHtml = item.cost > 0
                ? `<button class="shop-buy-btn" data-theme="${item.id}" data-cost="${item.cost}"><span>${item.cost}</span></button>`
                : `<button class="theme-buy-btn" data-theme="${item.id}">${getTranslation('select')}</button>`;
            return `
                <div class="shop-product-card card ${isAcquired ? 'acquired' : ''}" data-theme-id="${item.id}">
                    <div class="shop-product-card-image" style="background-image: linear-gradient(135deg, ${colors.start}, ${colors.end});">
                        <i data-lucide="${item.icon}" style="color: ${colors.icon}"></i>
                    </div>
                    <div class="shop-product-card-info">
                        <p>${getTranslation(item.nameKey)}</p>
                        ${buttonHtml}
                    </div>
                </div>`;
        };
        ...
        const renderThemeItem = (item) => {
            ...
            const colors = previewColors[item.color] || previewColors.gray;
            const buttonHtml = (item.cost > 0 && !isUnlockedByAchievement)
                ? `<button class="shop-buy-btn theme-buy-btn list-view-buy-btn" data-theme="${item.id}" data-cost="${item.cost}">
                       <span>${item.cost}</span>
                   </button>`
                : `<button class="theme-buy-btn" data-theme="${item.id}" data-achievement="${item.achievement || ''}">${getTranslation('select')}</button>`;
            return `
    ...
    function resetMiniPlayerUI() {
        ...
        if (miniPlayer) {
        // CORRECTION: On utilise un setTimeout pour s'assurer que le DOM est prêt avant d'appliquer
        // la hauteur, ce qui évite que le contenu soit coupé au premier chargement.
        setTimeout(() => {
            document.documentElement.style.setProperty('--mobile-player-height', '66px');
        }, 100); // Un petit délai suffit
    ...
    function setupEventListeners() {
        ...
        const allOverlays = document.querySelectorAll('#settings-overlay, #wifi-overlay, #tags-filter-overlay, #playlist-overlay, #player-options-overlay, #tutorial-overlay, #notifications-overlay, #reco-playlist-options-overlay, #queue-overlay, #mobile-queue-panel-backdrop');
        ...
        // CORRECTION: L'écouteur est maintenant sur le conteneur de la boutique pour gérer tous les clics
        // sur les boutons d'achat ou de sélection, même s'ils sont ajoutés dynamiquement.
        ...
        if (shopContainer) {
            shopContainer.addEventListener('click', (e) => {
                ...
                if (selectBtn && !selectBtn.classList.contains('locked') && !selectBtn.classList.contains('selected')) {
                    ...
                    playAudio(sounds.select);
                    return; // Arrête l'exécution pour ne pas traiter un double clic
                }

                if (buyBtn) {
                    // La logique d'achat sera implémentée ici plus tard
                    console.log('Bouton d\'achat cliqué:', buyBtn.dataset.theme, 'coût:', buyBtn.dataset.cost);
                }
            });
        }
        ...
        if (mainContentArea) {
            ...
            mainContentArea.addEventListener('touchstart', (e) => {
                ...
                // CORRECTION: On ignore le swipe s'il commence sur un élément qui a son propre défilement horizontal.
                if (e.target.closest('.horizontal-scroll-container, .carousel-container, #library-tabs-container')) return;
            ...
            mainContentArea.addEventListener('touchend', (e) => {
                // CORRECTION: On vérifie à nouveau ici pour s'assurer que le swipe n'a pas commencé sur un élément interdit.
                if (e.target.closest('.horizontal-scroll-container, .carousel-container, #library-tabs-container')) return;
                if (e.changedTouches.length === 1) {
                    const touchEndX = e.changedTouches[0].clientX;
                    const touchEndY = e.changedTouches[0].clientY;
                    ...
                    const swipeThreshold = 50; // CORRECTION: Seuil réduit pour un swipe plus sensible
    ...
    function handleMenuNavigation(dataLink, updateHistory = true, context = null) {
        ...
        if (dataLink === 'home-dashboard-section') {
            ...
            // NOUVEAU: Mettre à jour l'image de la carte "À propos" sur le dashboard mobile
    ...
    function renderDashboardCarousel() {
        ...
        container.innerHTML = latestItems.map(item => {
            ...
            <div class="carousel-item" data-item-id="${item.id}">
                ...
                <div class="carousel-item-info">
                    <h3 class="scrolling-container"><span>${item.title}</span></h3>
                    <p>${description}</p>
        ...
        setupTitleScrollObserver('#dashboard-carousel-container .carousel-item-info h3 > span');
    }
