...
function renderShopItems() {
    ...
    themesContainer.innerHTML = visibleThemes.map(item => { // Rendu des thèmes en cartes
        ...
        if (item.id === 'theme-ds' && window.innerWidth > 952) {
             actionHtml = `<button class="theme-buy-btn locked" disabled style="opacity: 0.5; cursor: url('assets/cursors/unavailable.cur'), not-allowed; font-size: 0.6em; font-weight: 800; padding: 0 5px; width: auto;">MOBILE ONLY</button>`;
        } else if (isSelected) {
        ...
        return `
            <div class="shop-product-card card ${!isUnlocked ? 'locked' : ''}" data-item-id="${item.id}" data-mission-id="${item.missionId || ''}">
                <div class="card-image-container">
...
function applyInitialTheme() {
    ...
    let savedUiTheme = localStorage.getItem('ui-theme') || 'default'; // Thème d'UI par défaut
...
function setupEventListeners() {
    ...
    window.addEventListener('resize', () => {
        ...
        if (localStorage.getItem('ui-theme') === 'theme-ds' && window.innerWidth > 952) {
            applyTheme('default');
        }
