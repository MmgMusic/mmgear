    ...
    let isYouTubeApiLoading = false; // NOUVEAU: Flag pour le lazy loading de l'API YT
    ...
    async function loadDataAndInitialize() { 
        try {
            ...
            if (profile.bonus) Object.entries(profile.bonus).forEach(([key, val]) => { val.id = key; val.type = 'bonus'; }); // Assigner un type
        ...
        // L'initialisation se fait maintenant de manière asynchrone pour ne pas bloquer le rendu.
        setTimeout(initializeApp, 0);
        // CORRECTION: L'initialisation se fait maintenant de manière asynchrone pour ne pas bloquer le rendu.
        // Cela permet au navigateur de traiter d'autres tâches pendant que les données sont préparées.
        setTimeout(initializeApp, 0); 
    ...
    // NOUVEAU: Fonction pour charger l'API YouTube à la demande
    function loadYouTubeAPI() {
        return new Promise((resolve) => {
            ...
            // Si l'API est en cours de chargement, on attend
    ...
    async function playVideoWhenReady(item, playlistIds = [], index = -1, playbackOriginType = 'titles', forceTempPlay = false, fromAutoplay = false, isSwipe = false) {
        if (!item) return;
        ...
        // NOUVEAU: Lazy load de l'API YouTube
        // On s'assure que l'API est chargée avant de continuer.
        if (!window.YT || !window.YT.Player) {
            showDialog("Chargement du lecteur vidéo...");
            await loadYouTubeAPI();
            // Si les lecteurs ne sont pas initialisés, on le fait maintenant.
            if (!playerInitializationComplete) onYouTubeIframeAPIReady();
        }
    ...
    // NOUVEAU: Fonction centralisée pour changer la couleur des barres système sur mobile
    function setSystemBarsColor(color) {
        ...
        // CORRECTION: Utilise une couleur solide qui simule la transparence pour une meilleure compatibilité.
        const overlayColor = isDark ? '#0F141B' : '#808080';

        if (statusBarMeta) {
            statusBarMeta.setAttribute('content', overlayColor);
        }

        if (navBarMeta) {
            navBarMeta.setAttribute('content', overlayColor);
        }
    }
    ...
    function openOverlay(overlay, sound, keepMusicPlaying = false) {
        ...
        if (!keepMusicPlaying) {
            ...
            setSystemBarsColor(); // Assombrit les barres système pour correspondre à l'overlay
    ...
    function closeOverlay(sound) {
        ...
        // NOUVEAU: Restaure la couleur normale des barres système
    ...
    function applyTheme(themeName) {
        ...
        // NOUVEAU: Lazy load du CSS du thème si nécessaire
        const themeData = themes.find(t => t.id === themeName);
        if (themeData && themeData.css) {
            lazyLoadCSS(themeData.css);
        }
    ...
    function setupEventListeners() {
        ...
        artworkOverlay.addEventListener('click', (e) => {
            if (e.target.id === 'artwork-overlay') {
                ...
                // NOUVEAU: Restaure les couleurs des barres système en fonction du thème
                updateThemeColorMeta();
        ...
        miniPlayer.addEventListener('touchend', (e) => {
            ...
            // Gère le swipe vers le bas pour masquer le lecteur
            if (swipeDistance > 50) { // Swipe vers le bas de 50px
                document.body.classList.add('mobile-player-hidden');
                document.documentElement.style.setProperty('--mobile-player-height', '0px');
    ...
    function renderDashboardCarousel() {
        ...
        container.innerHTML = latestItems.map(item => {
            ...
            <div class="carousel-item" data-item-id="${item.id}">
                <img src="${getCorrectImagePath(item, 'thumb')}" alt="${item.title}" decoding="async" onerror="this.onerror=null;this.src='${getCorrectImagePath(item, 'full')}'" /> <!-- loading="lazy" RETIRÉ pour le carrousel -->
