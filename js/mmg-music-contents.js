...
function renderShopMissions() {
    ...
    container.innerHTML = Object.entries(achievements).map(([id, ach]) => {
        ...
        let statusHtml = '';
        let cardClasses = 'shop-list-item card';
        const iconHtml = `<i class="fas ${achievements[id].icon}"></i>`;
        let progressText = '';

        let currentProgress = 0;
        if (id === 'loopMaster') {
            currentProgress = Math.max(0, ...Object.values(progress).map(Number));
        } else if (id === 'retroPlayer' || id === 'psPlayer' || id === 'spotimonFan') {
        ...
        if (unlocked) {
            cardClasses += ' unlocked'; // Classe pour un style "débloqué"
            ...
            statusHtml = `<button class="theme-buy-btn selected" disabled>${checkIcon}</button>`;
            progressText = `<p class="shop-item-description">${getTranslation(`achievement_${id}_desc`)}</p>`;
        } else {
            const percentage = Math.min(100, Math.floor((currentProgress / goal) * 100));
            statusHtml = `<button class="theme-buy-btn locked mission-progress-text" disabled>${percentage}%</button>`;
            progressText = `<p class="shop-item-description">${getTranslation(`achievement_${id}_desc`)} (${currentProgress}/${goal})</p>`;
        }

        return `
                <div class="${cardClasses}" data-mission-id="${id}">
                    <div class="shop-list-item-icon-container" data-icon-for-mission="${id}">
                        ${iconHtml}
                    </div>
                    <div class="list-view-title">
                        <span class="font-medium text-sm sm:text-base text-text-color"><span>${getTranslation(`achievement_${id}_title`)}</span></span>
                        ${progressText}
                    </div>
...
function renderShopItems() {
    ...
    themesContainer.innerHTML = themes.map(item => {
        ...
        const descriptionText = isUnlocked
            ? getTranslation('themeUnlocked') // NOUVEAU: Affiche "Thème débloqué"
            : `${getTranslation('completeMission')} "${getTranslation(`achievement_${item.missionId}_title`)}"`; // Affiche la mission à compléter
        ...
        return `
                <div class="shop-list-item card ${!isUnlocked ? 'locked' : ''}" data-item-id="${item.id}" data-mission-id="${item.missionId || ''}">
                    ...
                    <div class="list-view-title">
                        ...
                        <p class="shop-item-description">${descriptionText}</p>
                    </div>
                    <div class="shop-list-item-action">
                        ${actionHtml}
                    </div>
...
function updateThemeColorMeta() {
    ...
    if (navBarMeta) {
        ...
        navBarMeta.setAttribute('content', isDark ? '#121212' : '#ffffff');
    }
}
...
function openCardMenu(event, itemId) {
    ...
    if (isMobile) {
        // === VERSION MOBILE : PANNEAU GLISSANT (SLIDING PANEL) ===

        // Création du backdrop
        const backdrop = document.createElement('div');
        backdrop.className = 'mobile-card-menu-backdrop';
        backdrop.onclick = closeCardMenu; // Ferme au clic sur le fond
        document.body.appendChild(backdrop);

        // Création du panneau
        const panel = document.createElement('div');
        panel.className = 'mobile-card-menu-panel';

        // Contenu du panneau
        panel.innerHTML = `
            <div class="mobile-queue-handle-wrapper">
                <div class="mobile-queue-handle"></div>
            </div>
            <div class="mobile-card-menu-header">
                <img src="${item.image}" alt="${item.title}" class="mobile-card-menu-img">
                <div class="mobile-card-menu-info">
                    <h3 class="mobile-card-menu-title">${item.title}</h3>
                    <p class="mobile-card-menu-artist">${item.artist}</p>
                </div>
            </div>
            <div class="mobile-card-menu-items">
                <button class="mobile-card-menu-item" onclick="handleMenuAction('like', '${itemId}')">
                ...
                <button class="mobile-card-menu-item" onclick="handleMenuAction('playlist', '${itemId}')">
                ...
                <button class="mobile-card-menu-item" onclick="handleMenuAction('playNext', '${itemId}')">
                    <i class="fas fa-step-forward"></i>
                    <span>${getTranslation('playNext') || 'Jouer juste après'}</span>
                </button>
                <button class="mobile-card-menu-item" onclick="handleMenuAction('queue', '${itemId}')">
        ...
        activeCardMenu = { backdrop, panel, isMobile: true };
...
function closeCardMenu() {
    ...
    if (activeCardMenu.isMobile) {
        // Fermeture version mobile (avec animation)
        const { backdrop, panel } = activeCardMenu;

        backdrop.classList.remove('visible');
        panel.classList.remove('visible');

        // Attendre la fin de l'animation pour supprimer du DOM
        setTimeout(() => {
        ...
        document.removeEventListener('click', closeCardMenuOutside);
    }
...
function setupEventListeners() {
    ...
    const mainMenuTutorialSteps = [
        { step: 1, selector: '.sidebar', textKey: 'tutorial_computer_1', position: 'pos-right', action: () => document.querySelector('.content-section-board').scrollTop = 0 },
        { step: 2, selector: '.sidebar-header-buttons', textKey: 'tutorial_computer_2' },
        { step: 3, selector: '.central-profiles-card', textKey: 'tutorial_computer_3', position: 'pos-bottom' },
        { step: 4, selector: '#sidebar-main-nav', textKey: 'tutorial_computer_4' },
        { step: 5, selector: '.sidebar-footer', textKey: 'tutorial_computer_5' },
        { step: 6, selector: '.top-bar', textKey: 'tutorial_computer_6', position: 'pos-bottom' },
        { step: 7, selector: '.search-bar', textKey: 'tutorial_computer_7' },
        { step: 8, selector: '.top-bar-right', textKey: 'tutorial_computer_8' },
        { step: 9, selector: '.carousel-card', textKey: 'tutorial_computer_9' },
        { step: 10, selector: () => document.querySelector('.dashboard-row-split > div:nth-child(1)'), textKey: 'tutorial_computer_10', position: 'pos-top' },
        { step: 11, selector: () => document.querySelector('.dashboard-row-split > div:nth-child(2)'), textKey: 'tutorial_computer_11', position: 'pos-top' },
        { step: 12, selector: () => document.querySelector('.dashboard-row-split > div:nth-child(3)'), textKey: 'tutorial_computer_12', position: 'pos-top' },
        { step: 13, selector: '#player-album-cover', textKey: 'tutorial_computer_13', position: 'pos-top' },
        { step: 14, selector: '.player-buttons', textKey: 'tutorial_computer_14', position: 'pos-top' },
        { step: 15, selector: '.player-right-controls', textKey: 'tutorial_computer_15', position: 'pos-top' }
    ];

    // NOUVEAU: Guide pour mobile
    const mobileMainMenuTutorialSteps = [
        { step: 1, selector: '.mobile-header-icons', textKey: 'tutorial_mobile_1', position: 'pos-bottom', action: () => { const el = document.querySelector('.content-section-board'); if (el) el.scrollTop = 0; } },
        { step: 2, selector: '#mobile-profile-switch', textKey: 'tutorial_mobile_2', position: 'pos-bottom' },
        { step: 3, selector: '.search-bar', textKey: 'tutorial_mobile_3' },
        { step: 4, selector: '.carousel-card', textKey: 'tutorial_mobile_4' },
        { step: 5, selector: () => document.querySelector('.dashboard-row-split > div:nth-child(1)'), textKey: 'tutorial_mobile_5', position: 'pos-top' },
        { step: 6, selector: () => document.querySelector('.dashboard-row-split > div:nth-child(2)'), textKey: 'tutorial_mobile_6', position: 'pos-top' },
        { step: 7, selector: () => document.querySelector('.dashboard-row-split > div:nth-child(3)'), textKey: 'tutorial_mobile_7', position: 'pos-top' },
        { step: 8, selector: '.dashboard-guide-section', textKey: 'tutorial_mobile_8', position: 'pos-top' },
        { step: 9, selector: '.dashboard-about-section', textKey: 'tutorial_mobile_9', position: 'pos-top' },
        { step: 10, selector: '#mobile-bottom-nav', textKey: 'tutorial_mobile_10', position: 'pos-top' }
    ];
...
document.body.addEventListener('click', (e) => {
    ...
    if (selectBtn) {
        ...
        // NOUVEAU: Treat buy button clicks explicitly
        actionTarget = buyBtn;
    } else if (card && !card.dataset.missionId) {
        // NOUVEAU: Click on card itself (not on a button), but exclude mission cards
        // For locked background cards, trigger purchase; for unlocked cards, trigger selection
...
function updateMobileHeader(sectionId) {
    ...
    const svgLogo = document.getElementById('mobile-header-svg-logo');

    if (!logoContainer || !titleElement || !svgLogo) return;

    const sectionTitles = {
        'home-dashboard-section': null, // null pour afficher le logo
    ...
    logoContainer.className = title ? 'mobile-header-logo show-title' : 'mobile-header-logo show-logo';
}
        ...
        return ` 
                <div class="card card-reco-playlist" data-item-id="${playlist.id}"> 
                    <a href="#" class="card-link-wrapper" data-album-id="${playlist.id}">
                        <div class="card-image-container">
                            <div class="card__image card-icon-bg reco-playlist-${playlist.id}"></div> 
                        </div>
                    </a>
                    <div class="card-info-container">
                        <div class="card__text">
...
function renderDashboardQueue() {
    ...
    if (userQueue.length === 0) {
        ...
        if (recentTracks.length > 0) {
            ...
            title.className = 'queue-suggestions-title';
