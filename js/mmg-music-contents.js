    ...
    function setupEventListeners() {
        ...
        document.querySelectorAll('.view-toggle-btn').forEach(button => {
            button.addEventListener('click', (e) => { // Add click listener to view toggle button
                ...
                if (targetContainerId === 'library-container') {
                    ...
                    // CORRECTION: Pour la vue des titres d'un album, on réutilise les données déjà chargées.
                    const activeProfile = document.querySelector('.profile-tab.active')?.dataset.project;
        ...
        // CORRECTION: La logique dépend maintenant du type de contenu (musique ou vidéo).
        document.getElementById('mobile-mini-player').addEventListener('click', () => {
            if (!currentPlayingItem) return;

            if (isMusicTitle(currentPlayingItem)) {
                // Si c'est un titre musical, on ouvre le lecteur plein écran.
