    ...
    async function loadInitialData() {
        try {
            ...
            siteData = { ...baseData, contentData: {} }; // Initialise avec la structure de base

            // Charger les données du profil par défaut (mmg-music)
            await loadProfileData('mmg-music');
            initializeApp();
    ...
    async function loadProfileData(profileId) {
        ...
        if (siteData.contentData[profileId]) {
            return;
        }

        try {
            const response = await fetch(`${profileId}-data.json`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const profileData = await response.json();
            
            // Injecter le contenu exclusif si c'est le bon profil
            if (profileId === 'mmg-music') {
                injectExclusiveContent(profileData);
            }

            // Assigner les IDs et construire l'objet de recherche global
            Object.entries(profileData.titles || {}).forEach(([key, val]) => val.id = key);
            Object.entries(profileData.gameplay || {}).forEach(([key, val]) => val.id = key);
            Object.entries(profileData.makingofs || {}).forEach(([key, val]) => val.id = key);
            Object.entries(profileData.albums || {}).forEach(([key, val]) => {
                val.id = key;
                val.type = 'album';
            });
            
            siteData.contentData[profileId] = profileData;
        } catch (error) {
            console.error(`Could not load data for profile ${profileId}:`, error);
        }
    }
    ...
    function injectExclusiveContent() {
        ...
        Object.assign(profileData.titles, exclusiveTracks);
    }
    ...
    function onPlayerStateChange(event) {
        ...
        if (event.data === YT.PlayerState.ENDED) {
            ...
            // Relancer le son keep-alive de manière agressive
            if (isBackgroundPlayEnabled && sounds.keepAlive) {
                sounds.keepAlive.play().catch(e => {
                    // L'échec ici est normal si l'utilisateur a interagi avec autre chose
                });
            }
    ...
    function updateMediaSession(item) {
        if (!('serviceWorker' in navigator) || !navigator.serviceWorker.controller) {
            console.warn("MediaSession update skipped: Service Worker not active.");
            return;
        }
        const activeProfile = document.querySelector('.profile-tab.active')?.dataset.project || 'mmg-music';
        ...
        const payload = {
            ...
            artwork: [{ src: getCorrectImagePath(item), sizes: '512x512', type: 'image/jpeg' }], // JPEG est plus commun
            playbackState: (playerState === YT.PlayerState.PLAYING) ? 'playing' : 'paused'
        };
    ...
    function renderCards(containerId, cardsData, cardType = 'generic') {
        ...
        if (Object.keys(cardsData).length === 0) {
            container.innerHTML = `<p style="text-align: center; width: 100%;">${getTranslation('noResults')}</p>`;
            return;
        }
        ...
        Object.entries(cardsData).forEach(([key, item]) => {
                ...
                card.innerHTML = `
                  <a href="#" class="card-link-wrapper" data-unlock-album="dream-season">
                    <div class="card-inner">
                      <div class="lock-overlay" loading="lazy">
                      ...
                      <div class="card__content">
                        ...
                        <div class="card__text">
                          ...
                          <p class="card__description">${getTranslation('unlockATrack', { unlockedCount: unlockedDreamSeasonTracks.length })}</p>
                        </div>
                      </div>
                    </div>
                  </a>
            ...
            if (cardType === 'menu' && activeProfile === 'mmg-music') {
                const icons = {
                    ...
                    'videoClips': { icon: 'fa-camera', bg: 'bg-clips' },
                    'makingOfs': { icon: 'fa-lightbulb', bg: 'bg-makingofs' },
                    'likedTitles': { icon: 'fa-heart', bg: 'bg-liked' }
                };
    ...
    function renderMusicTitleDetails(item) {
        ...
        document.getElementById('details-album-art').loading = 'lazy'; // Ajout du lazy loading
    ...
    function renderAssociatedVideos(musicItem) {
        ...
        const createVideoCard = (video) => {
            ...
            card.innerHTML = `
                <img src="${getCorrectImagePath(video)}" alt="${video.title}" loading="lazy" onerror="this.src='https://placehold.co/100x56/000/fff?text=Error';">
                <p>${video.title}</p>
            `;
            return card;
        };
    ...
    function resetToHome(forceRebuild = false) {
        playAudio(sounds.select);
    ...
    function initializeApp() {
        ...
        initUserData();
        buildSearchableItems(); // Construire la liste de recherche initiale
                        ...
                        if (event.data && event.data.action) {
                            ...
                            switch (event.data.action) {
                                ...
                                case 'stop':
                                    ...
                                    break;
                                case 'seekbackward':
                                    if(activePlayer) activePlayer.seekTo(Math.max(0, activePlayer.getCurrentTime() - 10), true);
                                    break;
                                case 'seekforward':
                                    if(activePlayer) activePlayer.seekTo(Math.min(activePlayer.getDuration(), activePlayer.getCurrentTime() + 10), true);
                                    break;
                            }
                        }
                    });
    ...
    function setupEventListeners() {
        ...
        document.querySelectorAll('.profile-tab').forEach(tab => {
            tab.addEventListener('click', async () => {
                const profileId = tab.dataset.project;
                // Charger les données du profil si elles ne sont pas déjà là
                ...
                resetToHome(true); // Passer true pour forcer la reconstruction de allSearchableItems
            });
        });
        ...
        // Utilisation du debouncing pour la recherche
        const debouncedSearch = debounce(updateVisibleCards, 300);
        ...
        // Document Picture-in-Picture API for floating player
        const pipButton = document.getElementById('pip-btn');
        let pipWindow = null;

        // L'API Document PiP n'est actuellement supportée que sur les navigateurs de bureau.
        const isDesktop = !/Mobi|Android/i.test(navigator.userAgent);

        if ('documentPictureInPicture' in window && isDesktop) {
            pipButton.classList.remove('hidden'); // Show the button if API is supported

            pipButton.addEventListener('click', async () => {
                try {
                    if (pipWindow) {
                        // La fenêtre est déjà ouverte, l'utilisateur doit la fermer manuellement.
                        // On pourrait appeler pipWindow.close() mais c'est une bonne sécurité.
                        console.log("La fenêtre PiP est déjà ouverte.");
                        return;
                    } 
                    
                    const playerContainer = document.getElementById('mp3-player-container');
                    pipWindow = await window.documentPictureInPicture.requestWindow({
                        width: playerContainer.offsetWidth,
                        height: playerContainer.offsetHeight,
                    });

                    // Copier les feuilles de style dans la fenêtre PiP
                    [...document.styleSheets].forEach((styleSheet) => {
                        try {
                            ...
                            console.warn('Impossible de copier la feuille de style vers la fenêtre PiP:', e);
                        }
                    });

                    // Cloner le lecteur au lieu de le déplacer
                    const playerClone = playerContainer.cloneNode(true);
                    pipWindow.document.body.append(playerClone);
                    // Assurer que le corps de la fenêtre PiP a le même thème que la page principale
                    pipWindow.document.body.className = document.body.className;

                    // Écouter la fermeture de la fenêtre PiP
                    pipWindow.addEventListener('pagehide', () => {
                        // Le clone est détruit avec la fenêtre, pas besoin de le déplacer en retour.
                        pipWindow = null;
                    });
                } catch (error) {
                    console.error("PiP Error:", error);
                    pipWindow = null; // Reset on error
                }
            });
        }
