document.addEventListener('DOMContentLoaded', () => {
    ...
    function onPlayerStateChange(event) {
        ...
        if (event.data === YT.PlayerState.ENDED) {
            ...
            if (isMusicTitle(currentPlayingItem)) {
                setTimeout(() => {
                    if (!backgroundMusic.paused && !isReloadingForAd) {
    ...
    function updateMediaSession(item) {
        ...
        // Gère l'état de lecture pour le système (ex: notifications, écran de veille)
        // Remarque : Les navigateurs mobiles mettent souvent la lecture en pause lorsque l'onglet n'est plus visible pour économiser la batterie.
        // La Media Session API est la méthode standard pour permettre à l'utilisateur de contrôler la lecture en arrière-plan.
        ...
        // Ajout des contrôles Précédent/Suivant dans les notifications
        ...
        // Ajout des contrôles pour avancer/reculer de 10s (si supporté par le mobile)
    ...
    function addToPlaylist(itemId) {
        ...
        playAudio(sounds.select);
        renderPlaylist();
        updateDetailsPlaylistButtonState(); // MODIFICATION : Mettre à jour l'état du bouton
    }
    ...
    // MODIFICATION : Nouvelle fonction pour gérer l'état du bouton "Ajouter à la playlist"
    ...
    function renderMusicTitleDetails(item) {
        ...
        updateDetailsPlaylistButtonState(); // MODIFICATION : Mettre à jour le bouton à chaque affichage
    }
    ...
    function initializeApp() {
        ...
        if (initialSectionId) {
            ...
            if (sectionElement && sectionElement.classList.contains('page-section')) {
                sectionToLoad = initialSectionId;
            }
        }
        
        // MODIFICATION : Empêche le retour à une page "vide" avant le menu principal
        // Si aucune section n'est dans l'URL, on remplace l'historique au lieu d'en ajouter une nouvelle entrée.
        if (!window.location.hash || window.location.hash === "#") {
            history.replaceState({ section: 'menu-cards-section' }, '', '#menu-cards-section');
        }

        handleMenuNavigation(sectionToLoad, false); // Charge le contenu sans ajouter à l'historique
    }
    ...
    function setupEventListeners() {
        ...
        document.querySelector('.back-btn').addEventListener('click', (e) => {
            ...
            // MODIFICATION : Empêche le retour en arrière si on est sur le menu principal
            const menuSection = document.getElementById('menu-cards-section');
            if (menuSection && !menuSection.classList.contains('hidden')) {
                return; // Ne fait rien pour éviter de recharger la page
            }
        ...
        window.addEventListener('hashchange', () => {
            ...
            const sectionId = window.location.hash.substring(1) || 'menu-cards-section';
        ...
        contentBoard.addEventListener('click', (e) => {
            ...
            // MODIFICATION : Ajout de la gestion du clic pour le nouveau bouton
    ...
    function handleMenuNavigation(dataLink, updateHistory = true) {
        ...
        // Default to home if link is invalid
