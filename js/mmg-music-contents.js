    ...
    function renderCards(containerId, cardsData, cardType = 'generic') {
        ...
        // CORRECTION: Appliquer la vue sauvegardée (par défaut 'grid') et mettre à jour l'icône SVG.
        const savedView = localStorage.getItem(`mmg-view-${containerId}`) || 'grid';
        container.classList.toggle('list-view', savedView === 'list');
        container.classList.toggle('titles-grid', savedView === 'grid');
        ...
        Object.entries(cardsData).forEach(([key, item]) => {
                      ...
                      <div class="card__content">
                        ...
                        <div class="card__text">
                          ...
                          <p class="card__description">${(() => {
                              ...
                              return getTranslation(langKey, { count: lockedCount });
                          })()}</p>
                        </div>
                      </div>
                    </div>
            ...
            if (cardType === 'menu') { // S'applique maintenant aux deux profils
                    ...
                    switch (cardType) {
                        ...
                        case 'album':
                            ...
                            if (tracksInAlbum.length > 0) {
                                ...
                                if (lockedCount > 0) {
                                    const langKey = lockedCount === 1 ? 'trackToUnlock_one' : 'trackToUnlock_many';
                                    // NOUVEAU: Ajoute une classe pour cibler ce texte en CSS
                                    if (card) {
                ...
                // CORRECTION: On utilise les tags comme sous-titre
            ...
            // CORRECTION: Nouvelle structure HTML pour les cartes
            ...
            card.innerHTML = `
                <a href="#" class="card-link-wrapper" ${datasetAttributes}>
                    <div class="card-image-container">${badgeHtml}${cardImageHtml}</div>
                </a>
                <div class="card-info-container">
                    <div class="card__text">${cardTextHtml}</div>
                    ${isActionable ? `
                    <div class="card-actions">
    ...
    function setupEventListeners() {
        ...
        // CORRECTION: Gestionnaire pour le bouton de bascule de vue unique
        document.querySelectorAll('.view-toggle-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                // Détermine la nouvelle vue et bascule les classes
                const isList = container.classList.toggle('list-view');
                container.classList.toggle('titles-grid', !isList);
                const newView = isList ? 'list' : 'grid';

                // Change l'icône pour représenter la vue suivante
                if (isList) { // On est en mode liste, on montre l'icône pour passer en grille
                ...
                } else { // On est en mode grille, on montre l'icône pour passer en liste
                    icon.innerHTML = `<path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16"></path>`;
                }

                // Sauvegarde la préférence
                localStorage.setItem(`mmg-view-${targetContainerId}`, newView);
                playAudio(sounds.select);
            });
        });
    ...
    function handleMenuNavigation(dataLink, updateHistory = true) {
        ...
        if (dataLink === 'home-dashboard-section') {
            ...
            currentViewContext = { type: 'library', data: null };
            renderLibraryPage(); // Affiche la bibliothèque avec l'onglet "Likes" par défaut
    ...
    function renderLibraryPage(activeTabId = 'liked') {
        ...
        const titleElement = document.getElementById('library-playlist-title');
        const clearBtn = document.getElementById('library-clear-playlist-btn');

        if (!tabsContainer || !listContainer || !titleElement || !clearBtn) return;

        // 1. Rendre les onglets
        const tabs = [
            { id: 'liked', title: getTranslation('likedTitles') }, // Titres Likés
            { id: 'current', title: getTranslation('currentPlaylist') } // Playlist actuelle
        ];
        tabsContainer.innerHTML = tabs.map(tab => 
            `<button class="playlist-tab-btn ${tab.id === activeTabId ? 'active' : ''}" data-tab-id="${tab.id}">${tab.title}</button>`
        ).join('');

        // 2. Déterminer le contenu à afficher
        let itemsToShow = [];
        let isCustomPlaylist = false;
        ...
        // 4. Rendre la liste des titres (en utilisant renderCards pour la cohérence visuelle)
        ...
        // CORRECTION: Affiche un message spécifique si la playlist est vide.
        if (itemsToShow.length === 0) {
        ...
        // Ajouter les écouteurs sur les nouveaux onglets
