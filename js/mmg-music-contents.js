        ...
        panel.innerHTML = `
            ...
            <div class="mobile-card-menu-items">
                <button class="mobile-card-menu-item" onclick="handleMenuAction('like', '${itemId}', this)">
                    ...
                    <span>${isLiked ? (getTranslation('removeLike') || 'Ne plus aimer') : (getTranslation('like') || 'Liker')}</span>
                </button>
                <button class="mobile-card-menu-item" onclick="handleMenuAction('playlist', '${itemId}', this)">
                    ...
                    <span>${isInPlaylist ? (getTranslation('removePlaylist') || 'Retirer') : (getTranslation('addPlaylist') || 'Ajouter')}</span>
                </button>
                ...
                <button class="mobile-card-menu-item" onclick="handleMenuAction('queue', '${itemId}', this)">
                    ...
                    <span>${getTranslation('addToQueue') || 'File'}</span>
                </button>
    ...
    if (action === 'like') {
        ...
        if (buttonElement) {
            ...
            if (icon) {
                ...
                buttonElement.style.pointerEvents = 'none'; // Disable button temporarily
                setTimeout(() => {
...
function renderAssociatedVideos(musicItem) {
    ...
    const createVideoCard = (video, labelKey) => {
        const card = document.createElement('div');
        card.className = 'associated-video-card card-link-wrapper';
        card.dataset.youtubeId = video.youtube_id;
        card.dataset.itemId = video.id;
        const label = getTranslation(labelKey);
        card.innerHTML = `
                <img src="${getCorrectImagePath(video)}" alt="${label}" onerror="this.src='https://placehold.co/100x56/000/fff?text=Error';">
                <p>${label}</p>
            `;
        return card;
    };
        ...
        musicItem.associatedVideos.forEach(videoId => {
            ...
            if (video) {
                if (siteData.contentData[activeProfile].videos[videoId]) {
                    container.appendChild(createVideoCard(video, 'videoClip'));
                    foundVideo = true;
                } else if (siteData.contentData[activeProfile].bonus[videoId]) {
                    container.appendChild(createVideoCard(video, 'videoMakingOf'));
                    foundVideo = true;
                    hasMakingOf = true;
                }
            }
    ...
    if (!hasMakingOf) {
        ...
        ghostCard.className = 'associated-video-card card-link-wrapper ghost-card';
        ghostCard.innerHTML = `
            <div class="ghost-image-placeholder">
                <i class="fas fa-clock"></i>
            </div>
            <p>${getTranslation('videoMakingOf')}</p>
        `;
        container.appendChild(ghostCard);
        foundVideo = true; // To avoid "No associated video" message if we show a ghost card
    }

    if (!foundVideo) {
        container.innerHTML = `<p>${getTranslation('noAssociatedVideo')}</p>`;
    }
}
