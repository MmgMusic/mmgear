    ...
    function resetMiniPlayerUI() {
        ...
        if (miniPlayer) {
            // CORRECTION: Définit la hauteur du lecteur pour que le padding du contenu s'applique correctement au chargement.
            document.documentElement.style.setProperty('--mobile-player-height', '66px');
    ...
    function setupEventListeners() {
        ...
        const allOverlays = document.querySelectorAll('#settings-overlay, #shop-overlay, #wifi-overlay, #achievements-overlay, #tags-filter-overlay, #playlist-overlay, #player-options-overlay, #tutorial-overlay, #notifications-overlay, #reco-playlist-options-overlay, #queue-overlay, #mobile-queue-panel-backdrop');
    ...
    function renderSocials() {
        ...
        const platformConfig = {
            ...
            appleMusic: { icon: 'fab fa-apple', name: 'Apple Music' },
            deezer: { icon: 'fab fa-deezer', name: 'Deezer' }
        };
        ...
        container.innerHTML = Object.entries(socialLinks).map(([key, linkData]) => {
            ...
            if (!config) return '';
            return `
                <a href="${linkData.url}" target="_blank" class="social-link-new ${key}">
                    <i class="${config.icon}"></i> <span>${linkData.user}</span>
                </a>
            `;
        }).join('');
    }
