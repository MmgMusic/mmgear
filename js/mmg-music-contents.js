    ...
    function closeOverlay(sound) { // MODIFICATION: Refactorisation pour une fermeture plus fiable
        if (!activeOverlay) return;

        const overlayToClose = activeOverlay; // Sauvegarde une référence
        const isMobile = window.innerWidth <= 952;
        const settingsCard = overlayToClose.querySelector('.settings-card');

        const finishClosing = () => {
            // Gère les sons de connexion WiFi
            ...
            overlayToClose.classList.add('hidden'); // Cache l'overlay
            updateThemeColorMeta(); // Restaure la couleur des barres système
            activeOverlay = null; // Réinitialise la variable globale

            if (sound) playAudio(sound);

            // Logique pour reprendre la musique si elle était en pause pour l'overlay
        ...
        // Applique l'animation de glissement pour les panneaux mobiles
        if (isMobile && settingsCard && overlayToClose.id !== 'wifi-overlay' && overlayToClose.id !== 'purchase-confirm-overlay') {
            ...
            setTimeout(finishClosing, 400); // Attend la fin de l'animation
        } else {
            finishClosing(); // Fermeture immédiate
        }
    }
