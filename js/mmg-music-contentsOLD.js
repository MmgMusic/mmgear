    ...
    function onPlayerStateChange(event) {
        ...
        if (event.data === YT.PlayerState.ENDED) {
            ...
            // NOUVEAU: Logique pour la mission Spotimon
            if (currentPlayingItem.title.toLowerCase().includes('spotimon')) {
                updateAchievementProgress('spotimonFan', itemId);
            }
    ...
    function playVideoWhenReady(item, playlistIds = [], index = -1, playbackOriginType = 'titles', forceTempPlay = false, fromAutoplay = false, isSwipe = false) {
        ...
        if (itemIndexInUserQueue > -1) {
            ...
            // On met à jour l'affichage de la file d'attente sur le tableau de bord.
    ...
    function initUserData() {
        ...
        purchasedShopItems = storedPurchasedItems ? new Set(JSON.parse(storedPurchasedItems)) : new Set();
    ...
    function updateShopLocksAndSelection() { // CORRECTION: Logique entièrement revue
        ...
        document.querySelectorAll('.theme-buy-btn').forEach(btn => {
            ...
            if (achievementId && achievements[achievementId] && !achievements[achievementId].unlocked) {
                ...
                // Détermine si ce bouton correspond au thème (UI ou BG) actuellement sélectionné
                const isSelected = (themeId.includes('-bg') && themeId === currentBgTheme) || (!themeId.includes('-bg') && themeId === currentUiTheme);
                btn.innerHTML = isSelected ? `<i class="fas fa-check"></i> ${getTranslation('selected')}` : getTranslation('select');
    ...
    function renderShopPage() { // Render shop page
        ...
        // CORRECTION: Appelle la mise à jour des états des boutons juste après les avoir créés.
        updateShopLocksAndSelection();
    ...
    function renderShopItems() {
        ...
        if (!themesContainer || !backgroundsContainer) return;

        // Définition des couleurs pour les aperçus
        const previewColors = {
            blue: { start: '#3b82f6', end: '#60a5fa', icon: '#dbeafe' },
            orange: { start: '#f97316', end: '#fb923c', icon: '#ffedd5' },
            purple: { start: '#8b5cf6', end: '#a78bfa', icon: '#ede9fe' },
            indigo: { start: '#4f46e5', end: '#818cf8', icon: '#e0e7ff' },
            green: { start: '#16a34a', end: '#4ade80', icon: '#dcfce7' },
            yellow: { start: '#f59e0b', end: '#facc15', icon: '#fef3c7' },
            gray: { start: '#6b7280', end: '#9ca3af', icon: '#e5e7eb' },
        };

        const shopItems = {
            themes: [
                { id: 'default', nameKey: 'theme_default', descKey: 'theme_default_desc', icon: 'component', color: 'blue', cost: 0, achievement: null },
                { id: 'theme-8bit', nameKey: 'theme_8bit', descKey: 'theme_8bit_desc', icon: 'square', color: 'orange', cost: 3, achievement: 'retroPlayer' },
                { id: 'theme-16bit', nameKey: 'theme_16bit', descKey: 'theme_16bit_desc', icon: 'box', color: 'purple', cost: 3, achievement: 'loopMaster' },
                { id: 'theme-ps-style', nameKey: 'theme_ps_style', descKey: 'theme_ps_style_desc', icon: 'gamepad-2', color: 'indigo', cost: 3, achievement: 'psPlayer' },
                { id: 'theme-android', nameKey: 'theme_android', descKey: 'theme_android_desc', icon: 'smartphone', color: 'green', cost: 0, achievement: 'patienceIsKey' },
                { id: 'theme-spotimon', nameKey: 'theme_spotimon', descKey: 'theme_spotimon_desc', icon: 'pocket', color: 'red', cost: 0, achievement: 'spotimonFan' }, // NOUVEAU: Lié à la nouvelle mission
            ],
            backgrounds: [
                { id: 'default-bg', nameKey: 'bg_bubbles', icon: 'sparks', color: 'blue', cost: 0, image: 'assets/backgrounds/Background 1.webp' },
                { id: 'theme-bg-waves', nameKey: 'bg_waves', icon: 'waves', color: 'yellow', cost: 3, image: 'assets/backgrounds/Background 2.webp' },
                { id: 'theme-bg-gray', nameKey: 'bg_gray', icon: 'square', color: 'gray', cost: 3, image: 'assets/backgrounds/Background 3.webp' },
            ]
        };

        const renderBackgroundItem = (item) => {
            const isPurchased = purchasedShopItems.has(item.id);
            // CORRECTION: Utilisation du SVG pour l'icône de pièce pour une meilleure clarté.
            ...
            const buttonHtml = (item.cost > 0 && !isPurchased)
                ? `<button class="shop-buy-btn" data-theme="${item.id}" data-cost="${item.cost}">${coinIconSvg} <span>${item.cost}</span></button>`
                // CORRECTION: Utilisation du SVG pour l'icône de cercle pour une taille cohérente.
                : `<button class="theme-buy-btn" data-theme="${item.id}"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-circle"><circle cx="12" cy="12" r="10"/></svg></button>`;

            // CORRECTION: La carte utilise maintenant la structure standard des cartes (.card) pour hériter des styles de taille.
            return `
                <div class="shop-product-card card ${isPurchased ? 'unlocked' : ''}" data-theme-id="${item.id}">
                    <div class="card-image-container">
                        <img src="${item.image}" alt="Aperçu de ${getTranslation(item.nameKey)}" class="card__image">
                    </div>
                    <div class="card-info-container">
                        <div class="card__text">
                            <!-- CORRECTION: Le lien <a> a été retiré pour éviter les conflits de clics. Le clic est maintenant géré uniquement par les boutons. -->
                            <p class="card__title shop-product-title"><span>${getTranslation(item.nameKey)}</span></p>
                        </div>
                        ${buttonHtml}
                    </div>
                </div>`;
        ...
        const renderThemeItem = (item) => {
            // CORRECTION: La logique est simplifiée. Les thèmes sont débloqués par les succès, pas par achat.
            const isUnlockedByAchievement = item.achievement && achievements[item.achievement]?.unlocked;
            // CORRECTION: Utilisation de SVG pour les icônes pour une taille cohérente.
            ...
            // Si le thème a un coût ou n'est pas débloqué par un succès, il est verrouillé. Sinon, on peut le sélectionner.
            const buttonHtml = (item.achievement && !isUnlockedByAchievement)
                ? `<button class="theme-buy-btn locked" data-theme="${item.id}" data-achievement="${item.achievement || ''}" disabled>${lockIcon}</button>`
                : `<button class="theme-buy-btn" data-theme="${item.id}" data-achievement="${item.achievement || ''}">${circleIcon}</button>`;
            return `
                <div class="shop-list-item card" data-theme-id="${item.id}">
                    <div class="shop-list-item-icon-container">
                        <i data-lucide="${item.icon}"></i>
                    </div>
                    <div class="list-view-title">
                        <span class="font-medium text-sm sm:text-base text-text-color"><span>${getTranslation(item.nameKey)}</span></span>
                        <p class="shop-item-description">${getTranslation(item.descKey)}</p>
                    </div>
        ...
        const renderItem = (item, type) => {
            const isAcquired = (type === 'themes' && (localStorage.getItem('ui-theme') || 'default') === item.id) || 
                               (type === 'backgrounds' && (localStorage.getItem('bg-theme') || 'default-bg') === item.id) || // CORRECTION: Parenthèse fermante en trop retirée
                               (item.achievement && achievements[item.achievement]?.unlocked); // CORRECTION: Parenthèse fermante en trop retirée
            return `
            `;
        };

        themesContainer.innerHTML = shopItems.themes.map(renderThemeItem).join('');
        backgroundsContainer.innerHTML = shopItems.backgrounds.map(renderBackgroundItem).join('');
    ...
    function renderCards(containerId, cardsData, cardType = 'generic') {
        ...
        Object.entries(cardsData).forEach(([key, item], index) => {
            ...
            const imagePath = item.image ? getCorrectImagePath(item) : 'https://placehold.co/200x200/9E9E9E/FFFFFF?text=Playlist';
    ...
    function updateShopLocksAndSelection() {
        ...
        document.querySelectorAll('.theme-buy-btn').forEach(btn => {
            ...
            if (achievementId && achievements[achievementId] && !achievements[achievementId].unlocked) {
                ...
                btn.classList.remove('locked');
                const isSelected = (themeId.includes('-bg') && themeId === currentBgTheme) || (!themeId.includes('-bg') && themeId === currentUiTheme);
                // CORRECTION: Utilisation de SVG pour les deux états pour une taille cohérente.
                ...
                const circleIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-circle"><circle cx="12" cy="12" r="10"/></svg>`;
                btn.innerHTML = isSelected ? checkIcon : circleIcon;
                btn.classList.toggle('selected', isSelected);
                btn.disabled = isSelected;
            }
        });
    ...
    function renderSidebarNav(activeProfile) {
        ...
        const iconMap = {
            'albums': 'fa-compact-disc', 'videos': 'fa-video', 'bonus': 'fa-lightbulb',
            'about': 'fa-info-circle', 'albumsBeats': 'fa-compact-disc',
            'videosBeats': 'fa-video'
        };
    ...
    function setupTitleScrollObserver(containerSelector) {
        ...
        // CORRECTION: Le carrousel a son propre défilement horizontal, l'IntersectionObserver ne fonctionne pas.
        // On applique donc la vérification directement pour les titres du carrousel.
        if (containerSelector.includes('carousel-item-info')) {
            document.querySelectorAll(containerSelector).forEach(titleSpan => checkTitleOverflow(titleSpan));
            // On ne continue pas avec les observers pour ce cas spécifique.
            return;
        }
    ...
    // =========================================================
    const playNextTrack = (direction = 1, forcePlay = false, isSwipe = false) => { // CORRECTION: Syntaxe de la fonction corrigée
    ...
    function setupEventListeners() {
        ...
        mainWrapper.addEventListener('click', (e) => { // Add click listener to main wrapper
            ...
            // CORRECTION: La logique de la boutique est déplacée ici pour garantir qu'elle fonctionne.
            const buyBtn = e.target.closest('.shop-buy-btn');
            const selectBtn = e.target.closest('.theme-buy-btn');

            // NOUVEAU: Gère le clic sur un thème verrouillé
            ...
            if (buyBtn) {
                e.preventDefault();
                e.stopPropagation();
                const itemId = buyBtn.dataset.theme;
                const itemCost = parseInt(buyBtn.dataset.cost, 10);
                const itemNameEl = document.querySelector(`.card[data-theme-id="${itemId}"] .card__title span, .card[data-theme-id="${itemId}"] .list-view-title span span`);
                if (!itemNameEl) return;
                const itemName = itemNameEl.textContent;

                const confirmOverlay = document.getElementById('purchase-confirm-overlay');
                ...
                detailsContainer.innerHTML = `
                    <p>${getTranslation('purchaseMessage', { itemName: itemName, itemCost: itemCost })}</p>
                    <p>${getTranslation('yourCoins', { userCoins: userCoins })}</p>
                `;
                ...
                const handleConfirm = () => {
                    if (userCoins >= itemCost) {
                        ...
                        renderShopItems();
                        updateShopLocksAndSelection();
                        closeOverlay(sounds.coin);
                        showDialog(`Achat réussi ! Il vous reste ${userCoins} pièces.`);
                    } else {
                        errorMsg.textContent = getTranslation('notEnoughCoins');
                        errorMsg.classList.remove('hidden');
                        playAudio(sounds.blocked);
                    }
                };

                const handleCancel = () => closeOverlay(sounds.back);

                confirmBtn.addEventListener('click', handleConfirm, { once: true });
                cancelBtn.addEventListener('click', handleCancel, { once: true });

                openOverlay(confirmOverlay, sounds.select, true);
                return;
            }

            // CORRECTION: La logique de sélection des thèmes et des arrière-plans est maintenant unifiée.
            if (selectBtn && !selectBtn.classList.contains('locked') && !selectBtn.classList.contains('selected')) {
                e.preventDefault();
                e.stopPropagation();
                applyTheme(selectBtn.dataset.theme);
                playAudio(sounds.select);
            }
        ...
        // NOUVEAU: Logique pour le curseur de volume mobile
        ...
        if (mobileVolumeSlider) {
            const handleVolumeDrag = (e) => {
                ...
                const rect = mobileVolumeSlider.getBoundingClientRect();
                // Gère à la fois les événements de souris (clientX) et de toucher (touches[0].clientX)
                const clientX = e.clientX ?? e.touches[0].clientX;
                let newVolume = ((clientX - rect.left) / rect.width) * 100;
                newVolume = Math.max(0, Math.min(100, newVolume)); // Contraint la valeur entre 0 et 100
            ...
            mobileVolumeSlider.addEventListener('mousedown', (e) => {
                handleVolumeDrag(e); // Applique le volume au clic initial
                document.addEventListener('mousemove', handleVolumeDrag);
                document.addEventListener('mouseup', stopVolumeDrag);
            });

            mobileVolumeSlider.addEventListener('touchstart', (e) => {
                handleVolumeDrag(e); // Applique le volume au toucher initial
        ...
        const mobileVolumeDown = document.querySelector('#mobile-player-volume-control .fa-volume-down');
        if (mobileVolumeDown) {
            mobileVolumeDown.addEventListener('click', () => {
                setVolume(currentVolume - 10, true);
            });
        }
        const mobileVolumeUp = document.querySelector('#mobile-player-volume-control .fa-volume-up');
        if (mobileVolumeUp) {
        ...
        document.getElementById('settings-btn').addEventListener('click', (e) => { e.preventDefault(); openOverlay(document.getElementById('settings-overlay'), sounds.select, true); }); // Fix: Added missing closing parenthesis
        ...
        if (mobileShopBtn) mobileShopBtn.addEventListener('click', (e) => { 
            e.preventDefault(); 
            handleMenuNavigation('shop-section'); 
        });
        ...
        allOverlays.forEach(overlay => {
            // NOUVEAU: Gestion du clic sur le bouton shop
        ...
        document.querySelectorAll('.view-toggle-btn').forEach(button => {
            button.addEventListener('click', (e) => { // Add click listener to view toggle button
                ...
                if (targetContainerId === 'library-container') {
                    ...
                    const activeProfile = document.querySelector('.profile-tab.active')?.dataset.project;
                    const dataKey = targetContainerId.replace('-cards', ''); // ex: 'videos' ou 'bonus'
                    const sectionData = siteData.contentData[activeProfile][dataKey] || siteData.contentData[activeProfile][dataKey.replace('s', '')]; // Gère 'videos' -> 'video'
        ...
        // NOUVEAU: Isoler les événements de toucher sur la zone de volume pour éviter les conflits de swipe.
        const volumeWrapper = document.querySelector('.mobile-player-volume-wrapper');
        if (volumeWrapper) {
            volumeWrapper.addEventListener('touchstart', (e) => e.stopPropagation());
            volumeWrapper.addEventListener('touchmove', (e) => e.stopPropagation());
            volumeWrapper.addEventListener('touchend', (e) => e.stopPropagation());
        }
    ...
    document.body.addEventListener('click', (e) => {
        ...
        // Gère le clic sur un thème verrouillé
        if (selectBtn && selectBtn.classList.contains('locked')) {
            e.preventDefault();
            e.stopPropagation();
            const achievementId = selectBtn.dataset.achievement;
        ...
        // Gère l'achat d'un article (arrière-plan)
        if (buyBtn) {
            e.preventDefault();
            e.stopPropagation();
            const itemId = buyBtn.dataset.theme;
            ...
            detailsContainer.innerHTML = `<p>${getTranslation('purchaseMessage', { itemName: itemName, itemCost: itemCost })}</p><p>${getTranslation('yourCoins', { userCoins: userCoins })}</p>`;
            errorMsg.classList.add('hidden');
            ...
            const handleConfirm = () => {
                if (userCoins >= itemCost) {
                    ...
                    localStorage.setItem('mmg-purchasedItems', JSON.stringify([...purchasedShopItems]));
                    updateCoinDisplay();
                    renderShopItems(); // Re-render pour mettre à jour le bouton
                    updateShopLocksAndSelection(); // Re-render pour mettre à jour le bouton
                    closeOverlay(sounds.coin);
                    showDialog(`Achat réussi ! Il vous reste ${userCoins} pièces.`);
                } else {
                    errorMsg.textContent = getTranslation('notEnoughCoins');
            ...
            confirmBtn.addEventListener('click', handleConfirm, { once: true });
            cancelBtn.addEventListener('click', () => closeOverlay(sounds.back), { once: true });
    ...
    function handleMenuNavigation(dataLink, updateHistory = true, context = null) {
        ...
        const sections = {
            'videos-section': { data: getSectionData('videos-section') || profileContent.videos, type: 'video', container: 'videos-cards' },
            'bonus-section': { data: getSectionData('bonus-section') || profileContent.bonus, type: 'video', container: 'bonus-cards' },
    ...
    function renderDashboardCarousel() {
        ...
        setupCarousel();
        // MODIFICATION: Cible spécifiquement les spans des titres dans le carrousel pour le défilement.
    ...
    function setupCarousel() {
        ...
        // The scroll listener for the carousel should be inside setupCarousel
        let scrollTimeout;
        container.addEventListener('scroll', () => {
            ...
            scrollTimeout = setTimeout(() => {
                ...
                if (newSlideIndex !== currentSlide) {
                    ...
                    updateDots();
                    // Reset interval on manual swipe
                    clearInterval(carouselInterval);
                    carouselInterval = setInterval(nextSlide, 5000);
                }
            }, 100);
    ...
    // NOUVEAU: Logique de glisser-déposer unifiée pour PC et mobile
    function setupDraggableList(listContainer, handleSelector, itemSelector) {
        let draggedItem = null;
        ...
        const createPlaceholder = (item) => {
            ...
            p.className = 'queue-drag-placeholder';
        ...
        const onDragStart = (e) => {
            ...
            if (handle) {
                ...
                draggedItem.classList.add('queue-item-drag-source');
                const clientY = e.type === 'touchstart' ? e.touches[0].clientY : e.clientY;
                dragStartY = clientY - draggedItem.getBoundingClientRect().top;

                draggedItem.style.position = 'absolute';
                draggedItem.style.width = `${draggedItem.offsetWidth}px`;
                draggedItem.style.top = `${clientY - dragStartY}px`;
                draggedItem.style.zIndex = '1300';
        ...
        const onDragMove = (e) => {
            ...
            const otherItems = [...listContainer.querySelectorAll(`${itemSelector}:not(.queue-item-drag-source)`)];
            const afterElement = otherItems.find(child => {
        ...
        const onDragEnd = () => {
            ...
            draggedItem.classList.remove('queue-item-drag-source');
            ...
            const newOrderIds = [...listContainer.querySelectorAll('.playlist-item[data-item-id]')].map(el => el.dataset.itemId);
            userQueue = newOrderIds;
            draggedItem = null;
        ...
        listContainer.addEventListener('mousedown', onDragStart);
        listContainer.addEventListener('touchstart', onDragStart, { passive: false });
    }
    ...
    // Appliquer la logique de drag-and-drop aux deux files d'attente
    setupDraggableList(document.getElementById('mobile-queue-list'), '.mobile-queue-drag-handle', '.playlist-item-wrapper');
    setupDraggableList(document.getElementById('queue-container'), '.playlist-drag-handle', '.playlist-item');

    // CORRECTION: La fonction est déplacée ici pour être accessible globalement.
