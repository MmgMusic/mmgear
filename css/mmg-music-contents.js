...
// MODIFICATION: La fonction n'attribue plus le bonus, elle est appelée par le clic sur le bouton
function claimDailyBonus() {
    if (!isDailyBonusAvailable()) {
        console.log("Bonus déjà réclamé ou non disponible.");
        return;
    }
    ...
    if (loginStreak < 7) {
        ...
        if (lockedTracks.length > 1) {
            ...
            // Message pour le dernier titre (MODIFIÉ)
    ...
    if (rewardMessage && !rewardMessage.includes('Super Listener')) { // Ne pas afficher le dialog standard pour la récompense finale
...
// ANCIENNE FONCTION checkDailyBonus() - maintenant vide, on peut la supprimer ou la laisser pour référence.
function checkDailyBonus() {
    // Cette fonction est maintenant obsolète. La logique est dans isDailyBonusAvailable() et claimDailyBonus().
}
...
function renderDailyBonusProgress() {
    ...
    if (!section || !container || !description) return;
    ...
    if (isDailyBonusAvailable()) {
        // Une récompense est disponible : on affiche le bouton "Récupérer"
        container.innerHTML = `
            <button id="claim-bonus-btn" class="claim-bonus-button">
                <i class="fas fa-gift"></i>
                <span data-lang-key="claimReward">Récupérer la récompense</span>
            </button>
        `;
        description.textContent = getTranslation('dailyBonusReady');
        document.getElementById('claim-bonus-btn').addEventListener('click', claimDailyBonus);
        return; // On arrête ici si le bouton est affiché
    }
    ...
    let progressHtml = '';
    // Ajoute la barre de progression colorée
    progressHtml += `<div id="daily-bonus-progress-line-fill" style="width: ${progressPercentage}%;"></div>`;

    for (let i = 1; i <= 7; i++) {
        const isActive = i <= loginStreak;
        let rewardLabel = '';

        if (i < 7) {
            rewardLabel = `+${i} <i class="fas fa-coins"></i>`;
        } else { // Jour 7
            rewardLabel = `<i class="fas fa-gift"></i> ${getTranslation('specialReward')}`;
        }

        progressHtml += `
            <div class="daily-bonus-step ${isActive ? 'active' : ''}">
    ...
    // Met à jour le texte descriptif
    if (loginStreak === 6) { // MODIFIÉ: Utilise getTranslation
        description.textContent = getTranslation('dailyBonusNextRewardSpecial');
    } else if (loginStreak > 0) { // MODIFIÉ: Utilise getTranslation
        description.textContent = getTranslation('dailyBonusStreakText');
    } else { // MODIFIÉ: Utilise getTranslation
